...
tidy:
	...
	cd modules/observability/datadog && go mod tidy
...
graph: verify-tools
	...
	go tool godepgraph -s ./modules/observability/datadog | dot -Tpng -o ./docs/img/datadog.png
...
generate: verify-tools graph
	...
	cd modules/observability/datadog && go generate ./...
...
imports: verify-tools
	...
	cd modules/observability/datadog && go tool goimports-reviser -rm-unused -set-alias -format -recursive .
...
format: verify-tools
	...
	cd modules/observability/datadog && go fmt ./...
...
vet: verify-tools
	...
	cd modules/observability/datadog && vet ./...
...
lint:
	...
	cd modules/observability/datadog && go tool golangci-lint run --fix ./...
...
test: verify-tools
	...
	cd modules/observability/datadog && go test -covermode atomic -coverprofile .reports/testcoverage.out ./...
...
coverage: verify-tools test
	...
	cd modules/observability/datadog && go tool cover -html=.reports/testcoverage.out -o .reports/testcoverage.html && $(COVERAGE_BADGE) && go tool go-test-coverage --config=.testcoverage.yml || true
...
check: verify-tools
	...
	cd modules/observability/datadog && go tool govulncheck ./...
