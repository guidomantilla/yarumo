# Acceptance Tests — Prompt 03: invariants_numeric_test.go
...
You are a Go testing engineer. Generate the file `invariants_numeric_test.go`.

## Output

Generate exactly ONE file: `invariants_numeric_test.go`
Place it at: `modules/compute/tests/acceptance/invariants_numeric_test.go`
...
These are defined in `helpers_test.go` and available at compile time:
- `fuzzyGrid(step float64) []float64` — generates [0.0, step, ..., 1.0]
- `assertFloat(t *testing.T, name string, got, want, tolerance float64)` — float comparison
- Tolerance constants: `floatTolerance`, `probTolerance`, `defuzzTolerance`
...
The file must contain ALL of the following tests.

### Section 1.6: fuzzy/ — Axioms (14 tests)

Include the COMPLETE code for each test. Here are the tests with their full specifications:

**Test 1: TestAcceptance_TNorm_commutativity_exhaustive**

Grid exhaustive: T(a,b)==T(b,a) for Min/Product/Lukasiewicz, 441 pairs x 3 = 1323 verifications.
Reference: Zadeh (1965), Klir & Yuan.
...
**Test 2: TestAcceptance_TNorm_associativity_exhaustive**

Grid exhaustive: T(T(a,b),c)==T(a,T(b,c)), 9261 triples x 3 = 27783 verifications.
...
**Test 3: TestAcceptance_TNorm_monotonicity_exhaustive**

Grid exhaustive: a1<=a2 implies T(a1,b)<=T(a2,b), ~4600 per norm x 3.
...
**Test 4: TestAcceptance_TNorm_identity_exhaustive**

T(a,1.0)==a, 21 x 3 = 63 verifications.
...
**Test 5: TestAcceptance_TConorm_commutativity_exhaustive**

S(a,b)==S(b,a) for Max/ProbabilisticSum/BoundedSum, 441 x 3 = 1323 verifications.
...
**Test 6: TestAcceptance_TConorm_associativity_exhaustive**

S(S(a,b),c)==S(a,S(b,c)), 9261 x 3 = 27783 verifications.
...
**Test 7: TestAcceptance_TConorm_monotonicity_exhaustive**

a1<=a2 implies S(a1,b)<=S(a2,b), ~4600 x 3.
...
**Test 8: TestAcceptance_TConorm_identity_exhaustive**

S(a,0)==a, 21 x 3 = 63 verifications.
...
**Test 9: TestAcceptance_TNorm_Lukasiewicz_boundary**

Known-answer: Luk(0.3,0.6)=0, Luk(0.5,0.51)=0.01, Luk(1,1)=1. Uses assertFloat with 1e-15 tolerance.
...
**Test 10: TestAcceptance_TNorm_Product_epsilon**

Adversarial subnormal: Product(1e-300,1e-300)>=0, Product(1e-15,1e-15) approx 1e-30.
...
**Test 11: TestAcceptance_Membership_exact_values**

Known-answer: triangular peak/zeros, trapezoidal plateau, gaussian center/symmetry.
...
**Test 12: TestAcceptance_Defuzzification_output_in_range**

Boundary: all 5 methods (Centroid, Bisector, MeanOfMax, LargestOfMax, SmallestOfMax) produce output in [0,10] for triangular(0,5,10) with 100 samples.
...
**Test 13: TestAcceptance_Stats_known_answer_Normal**

Normal(0,1): PDF(0) approx 0.398942280401433, CDF(0)=0.5, CDF(1.96) approx 0.975002104852, CDF(-1.96) approx 0.024997895148, Mean=0, Variance=1.
...
**Test 14: TestAcceptance_Stats_known_answer_Exponential**

Exponential(2): PDF(0)=2, CDF(1) approx 0.864664716763387, Mean=0.5, Variance=0.25.
...
**Test 15: TestAcceptance_Stats_known_answer_Beta**
...
**Test 16: TestAcceptance_Stats_known_answer_StudentT**

StudentT(5): CDF(0)=0.5, Mean=0, Variance=5/3.
...
**Test 17: TestAcceptance_Stats_known_answer_Poisson**

Poisson(3): PMF(0) approx 0.049787068368, PMF(3) approx 0.224041807660, Mean=3, Variance=3.
...
**Test 18: TestAcceptance_Stats_known_answer_Binomial**

Binomial(10,0.3): PMF(3) approx 0.266827932, Mean=3, Variance=2.1.
...
**Test 19: TestAcceptance_Stats_adversarial_Welford**

Catastrophic cancellation [1e8+1,1e8+2,1e8+3] PopulationVariance=2/3; identical data variance=0; single datum variance=0 mean=42.
...
**Test 20: TestAcceptance_Stats_adversarial_WindowedStats**

Matches full recompute: windowSize=10, 100 deterministic values, manual mean/variance vs WindowedStats.
...
func TestAcceptance_Stats_adversarial_WindowedStats(t *testing.T) {
	...
	for i, v := range values {
		...
		t.Run(fmt.Sprintf("step_%d", i), func(t *testing.T) {
			...
			assertFloat(t, fmt.Sprintf("mean at step %d", i), gotMean, wantMean, floatTolerance)
			assertFloat(t, fmt.Sprintf("variance at step %d", i), gotVariance, wantVariance, 1e-6)
		})
	}
}
```

**Test 21: TestAcceptance_Stats_adversarial_Bayes**

Base rate fallacy: prior=0.001, sensitivity=0.99, falsePositive=0.05.
...
func TestAcceptance_Stats_adversarial_Bayes(t *testing.T) {
	...
	// P(H|E) = 0.00099 / 0.05094 = 0.019434...
...
**Test 22: TestAcceptance_Stats_known_answer_ChiSquared**

ChiSquared(3): PDF(1) approx 0.24197, CDF(7.815) approx 0.95, Mean=3, Variance=6.
...
func TestAcceptance_Stats_known_answer_ChiSquared(t *testing.T) {
	...
	t.Run("CDF_at_7.815", func(t *testing.T) {
		...
		assertFloat(t, "ChiSquared(3).CDF(7.815)", dist.CDF(7.815), 0.95, 1e-2)
	})
...
**Test 23: TestAcceptance_Stats_hypothesis_TTest**

Non-rejection: sample=[2.1,2.3,1.9,2.0,2.2] H0:mu=2.0; rejection: sample=[10.1,10.3,9.9,10.2,10.4,10.0,10.5] H0:mu=9.0.
...
**Total: 26 tests (14 fuzzy axioms + 12 stats distributions).**
...
go vet ./...
go test -run "TestAcceptance_(TNorm|TConorm|Membership|Defuzz|Stats)" -count=1 -v ./...
```
