# Acceptance Tests — Prompt 01: helpers_test.go
...
You are a Go testing engineer. Generate the file `helpers_test.go` for the
acceptance test module at `modules/compute/tests/acceptance/`.

## Output

Generate exactly ONE file: `helpers_test.go`

Place it at: `modules/compute/tests/acceptance/helpers_test.go`
...
The file must contain ALL of the following helpers and constants. Other test
files (invariants, golden, errors, performance) depend on these.
...
Builds all formulas up to the given depth using the provided variables.
...
- BinOps: {AndF, OrF, ImplF, IffF}
- Also generates reversed operand order for ImplF (non-commutative)
...
Builds the Rain-Sprinkler-WetGrass Bayesian network from Koller & Friedman.
...
Builds the CreditHistory-Default-IncomeLevel network for the loan approval scenario.
...
Builds a fuzzy risk assessment engine for the loan approval scenario.
...
Builds the canonical tipping fuzzy engine (Mamdani). Accepts optional engine options.
...
### generateEvidenceCombos

Generates all possible evidence subsets for the given variables.
...
### Structural Checkers for CNF/DNF

These are used by invariant tests to verify structural form.
...
The file should compile with no errors. It contains no test functions — only
helpers, constants, and structural checkers used by other test files.
