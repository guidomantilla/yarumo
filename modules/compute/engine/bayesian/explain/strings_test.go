...
func TestFactor_String(t *testing.T) {
	...
	f := Factor{Variables: []stats.Var{"A", "B"}, Size: 4}
	result := f.String()
...
func TestStep_String_withFactor(t *testing.T) {
	...
	s := Step{
		...
		Factor:  Factor{Variables: []stats.Var{"X"}, Size: 2},
	}
...
func TestPosterior_String(t *testing.T) {
	...
	p := Posterior{
		Variable:     "X",
		Distribution: stats.Distribution{"true": 0.7, "false": 0.3},
	}
...
func TestTrace_String_basic(t *testing.T) {
	...
	tr := NewTrace("X", stats.Assignment{"Y": "true"})
...
func TestFormatEvidence_sorted(t *testing.T) {
	...
	ev := stats.Assignment{"C": "c1", "A": "a1", "B": "b1"}
