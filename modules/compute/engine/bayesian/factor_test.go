...
func TestNewFactor(t *testing.T) {
	...
	table := map[string]stats.Prob{"A=t": 0.7, "A=f": 0.3}
	f := NewFactor([]stats.Var{"A"}, table)
...
func TestNewFactor_copiesInputs(t *testing.T) {
	...
	vars := []stats.Var{"A"}
	table := map[string]stats.Prob{"A=t": 0.7}
...
func TestMultiply_basic(t *testing.T) {
	...
	a := NewFactor([]stats.Var{"A"}, map[string]stats.Prob{
		"A=t": 0.6,
		"A=f": 0.4,
	})
	b := NewFactor([]stats.Var{"A", "B"}, map[string]stats.Prob{
		"A=t,B=t": 0.2,
		"A=t,B=f": 0.8,
		"A=f,B=t": 0.5,
		"A=f,B=f": 0.5,
	...
	expected := 0.6 * 0.2
	if math.Abs(float64(result.Table["A=t,B=t"])-expected) > 1e-9 {
...
func TestMultiply_conflictingAssignments(t *testing.T) {
	...
	b := NewFactor([]stats.Var{"A"}, map[string]stats.Prob{
		"A=f": 0.4,
	})
...
func TestSumOut_basic(t *testing.T) {
	...
	expected := 0.12 + 0.48
	if math.Abs(float64(result.Table["A=t"])-expected) > 1e-9 {
...
func TestRestrict_noMatch(t *testing.T) {
	...
	f := NewFactor([]stats.Var{"A"}, map[string]stats.Prob{
		"A=t": 0.6,
	})
...
func TestNormalizeFactor_basic(t *testing.T) {
	...
	if math.Abs(float64(result.Table["A=t"])-0.4) > 1e-9 {
		t.Fatalf("expected 0.4, got %f", float64(result.Table["A=t"]))
	}

	if math.Abs(float64(result.Table["A=f"])-0.6) > 1e-9 {
...
func TestMergeVars(t *testing.T) {
	...
	result := mergeVars([]stats.Var{"B", "A"}, []stats.Var{"C", "A"})
...
func TestRemoveVar(t *testing.T) {
	...
	result := removeVar([]stats.Var{"A", "B", "C"}, "B")
...
func TestMergeAssignments_noConflict(t *testing.T) {
	...
	a := stats.Assignment{"A": "t"}
	b := stats.Assignment{"B": "f"}
