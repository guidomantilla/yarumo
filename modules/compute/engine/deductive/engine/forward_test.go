...
func TestForward_oscillationDetection(t *testing.T) {
	t.Parallel()

	t.Run("stops early with contradictory rules", func(t *testing.T) {
		t.Parallel()

		// R1 sets B=true, R2 sets B=false. Both fire when A=true.
		// Without oscillation detection this would run for maxIterations.
		r1 := rules.NewRule("setTrue", logic.Var("A"), map[logic.Var]bool{"B": true}, rules.WithPriority(1))
		r2 := rules.NewRule("setFalse", logic.Var("A"), map[logic.Var]bool{"B": false}, rules.WithPriority(2))
		e := NewEngine(WithMaxIterations(1000))

		result := e.Forward(logic.Fact{"A": true}, []rules.Rule{r1, r2})

		// Should stop after 2 iterations (first pass changes state, second pass is net no-op).
		// Without fix, would use all 1000 iterations × 2 rules = 2000 steps.
		if result.Steps > 10 {
			t.Fatalf("expected oscillation detection to stop early, got %d steps", result.Steps)
		}
	})

	t.Run("last writer wins deterministically", func(t *testing.T) {
		t.Parallel()

		// Lower priority number = fires first. Higher priority fires second and wins.
		r1 := rules.NewRule("setTrue", logic.Var("A"), map[logic.Var]bool{"B": true}, rules.WithPriority(1))
		r2 := rules.NewRule("setFalse", logic.Var("A"), map[logic.Var]bool{"B": false}, rules.WithPriority(2))
		e := NewEngine()
		...
		snap := result.Facts.Snapshot()
		if snap["B"] {
			t.Fatal("expected B=false (last writer wins)")
		}
	})
}
