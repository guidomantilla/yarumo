...
func TestBackward_negatedCondition(t *testing.T) {
	t.Parallel()

	t.Run("proves goal via negated condition under CWA", func(t *testing.T) {
		t.Parallel()

		// Rule: NOT(A) => B=true. A is unknown (false under CWA), so NOT(A) is true.
		r := rules.NewRule("r1", logic.NotF{F: logic.Var("A")}, map[logic.Var]bool{"B": true})
		e := NewEngine()

		proven, result := e.Backward(logic.Fact{}, []rules.Rule{r}, "B")
		if !proven {
			t.Fatal("expected goal B to be proven via NOT(A) under CWA")
		}
	...
	t.Run("negated condition with known true variable", func(t *testing.T) {
		t.Parallel()

		// Rule: NOT(A) => B=true. A is known true, so NOT(A) is false.
		r := rules.NewRule("r1", logic.NotF{F: logic.Var("A")}, map[logic.Var]bool{"B": true})
		e := NewEngine()

		proven, _ := e.Backward(logic.Fact{"A": true}, []rules.Rule{r}, "B")
		if proven {
			t.Fatal("expected goal B not proven when A is true")
		}
	})

	t.Run("mixed positive and negated conditions", func(t *testing.T) {
		t.Parallel()

		// Rule: (X AND NOT(Y)) => Z=true. X is true, Y is unknown (false under CWA).
		r := rules.NewRule("r1",
			logic.AndF{L: logic.Var("X"), R: logic.NotF{F: logic.Var("Y")}},
			map[logic.Var]bool{"Z": true},
		)
		e := NewEngine()

		proven, result := e.Backward(logic.Fact{"X": true}, []rules.Rule{r}, "Z")
		if !proven {
			t.Fatal("expected goal Z to be proven via X=true AND NOT(Y) under CWA")
		}

		snap := result.Facts.Snapshot()
		if !snap["Z"] {
			t.Fatal("expected Z=true in facts")
		}
	})
}
