...
func NewEngine(inputVars, outputVars []variable.Variable, ruleSet []rules.Rule, opts ...Option) Engine {
	...
	ivCopy := make([]variable.Variable, len(inputVars))
	copy(ivCopy, inputVars)

	ovCopy := make([]variable.Variable, len(outputVars))
	copy(ovCopy, outputVars)

	rsCopy := make([]rules.Rule, len(ruleSet))
	copy(rsCopy, ruleSet)
