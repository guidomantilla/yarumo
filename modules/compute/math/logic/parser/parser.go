...
func ParseWith(input string, opts ...Option) (logic.Formula, error) {
	...
	if trimmed == "" {
		return nil, ErrParse(0, 1, ErrEmptyInput.Error(), ErrEmptyInput)
	}
	...
	if p.tokens[p.pos].kind != tokEOF {
		...
		return nil, ErrParse(tok.pos, tok.pos+1, ErrUnexpectedToken.Error()+": "+tok.val, ErrUnexpectedToken)
	}
...
func (p *parser) parseAtom() (logic.Formula, error) {
	...
	case tokLParen:
		return p.parseGroup()
	case tokEOF:
		return nil, ErrParse(tok.pos, tok.pos+1, ErrUnexpectedEnd.Error(), ErrUnexpectedEnd)
	default:
		return nil, ErrParse(tok.pos, tok.pos+1, ErrUnexpectedToken.Error()+": "+tok.val, ErrUnexpectedToken)
	}
}
...
func (p *parser) parseGroup() (logic.Formula, error) {
	...
	if p.peek().kind != tokRParen {
		return nil, ErrParse(p.peek().pos, p.peek().pos+1, ErrUnclosedParen.Error(), ErrUnclosedParen)
	}
