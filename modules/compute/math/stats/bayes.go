...
func Bayes(prior, likelihood, evidence Prob) (Prob, error) {
	if evidence == 0 {
		return 0, ErrZeroEvidence
	}

	return Prob(float64(likelihood) * float64(prior) / float64(evidence)), nil
}

// TotalProbability computes P(E) = Sum P(E|Hi)*P(Hi).
func TotalProbability(priors, likelihoods []Prob) (Prob, error) {
