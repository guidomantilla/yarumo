...
func newTestRedisCache(t *testing.T) (ccache.Cache[string, string], *miniredis.Miniredis) {
	...
	c := NewRedisCache[string]("test", WithAddr(server.Addr()))
...
func TestNewRedisCache(t *testing.T) {
	...
	t.Run("constructor itself does no I/O against an unreachable address", func(t *testing.T) {
		...
		c := NewRedisCache[string]("offline-ctor", WithAddr("127.0.0.1:1"))
		if c == nil {
...
func TestRedisCache_Start(t *testing.T) {
	...
	t.Run("returns ErrStart wrapping ErrRedisCommandFailed when PING fails", func(t *testing.T) {
		...
		c := NewRedisCache[string]("offline", WithAddr("127.0.0.1:1"))
		t.Cleanup(func() { _ = c.Stop(context.Background()) })
	...
	t.Run("succeeds against a reachable server", func(t *testing.T) {
		...
		server := miniredis.RunT(t)
		c := NewRedisCache[string]("reachable", WithAddr(server.Addr()))
		t.Cleanup(func() { _ = c.Stop(context.Background()) })
...
func TestRedisCache_Get(t *testing.T) {
	...
	t.Run("returns ErrRedisDecodeFailed when stored bytes do not decode into V", func(t *testing.T) {
		...
		c := NewRedisCache[int]("decode-test", WithAddr(server.Addr()))
...
func TestRedisCache_Set(t *testing.T) {
	...
	t.Run("returns ErrRedisEncodeFailed when codec cannot encode", func(t *testing.T) {
		...
		server := miniredis.RunT(t)
		c := NewRedisCache[chan int]("encode-test", WithAddr(server.Addr()))
...
func TestRedisCache_Clear(t *testing.T) {
	...
	t.Run("removes only entries under this cache's key prefix", func(t *testing.T) {
		...
		server := miniredis.RunT(t)
		c := NewRedisCache[string]("scoped", WithAddr(server.Addr()))
...
func TestRedisCache_StopIsIdempotent(t *testing.T) {
	...
	server := miniredis.RunT(t)
	c := NewRedisCache[string]("idempotent-stop", WithAddr(server.Addr()))
...
func TestRedisCache_KeyPrefix(t *testing.T) {
	...
	t.Run("default prefix uses name", func(t *testing.T) {
		...
		server := miniredis.RunT(t)
		c := NewRedisCache[string]("alpha", WithAddr(server.Addr()))
