...
func TestKeepAlive_Start(t *testing.T) {
	t.Parallel()

	t.Run("returns nil with live ctx", func(t *testing.T) {
		t.Parallel()

		c := NewKeepAlive("worker-start-1")
...
func TestKeepAlive_Stop(t *testing.T) {
	...
	t.Run("returns ErrShutdownTimeout when ctx already expired", func(t *testing.T) {
		...
		if !errors.Is(err, lifecycle.ErrShutdownFailed) {
...
func TestKeepAlive_Done(t *testing.T) {
	t.Parallel()

	t.Run("unblocks readers after Stop", func(t *testing.T) {
		t.Parallel()

		c := NewKeepAlive("worker-done-1")
