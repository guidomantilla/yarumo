...
const textCodecTestName = "JWT_HS256"
...
func TestMethod_MarshalText(t *testing.T) {
	...
	t.Run("returns the registry name", func(t *testing.T) {
		...
		data, err := JWT_HS256.MarshalText()
		if err != nil {
...
func TestMethod_UnmarshalText(t *testing.T) {
	...
	t.Run("returns ErrAlgorithmNotSupported for unknown name", func(t *testing.T) {
		...
		if domErr.Type != TokenMethod {
			t.Fatalf("expected type %q, got %q", TokenMethod, domErr.Type)
		}
	})
}
...
func TestMethod_TextCodec_RoundTrip(t *testing.T) {
	...
	t.Run("MarshalText then UnmarshalText preserves identity", func(t *testing.T) {
		...
		if got.Name() != JWT_HS256.Name() {
			t.Fatalf("expected %q, got %q", JWT_HS256.Name(), got.Name())
		}

		if got.signingMethod != JWT_HS256.signingMethod {
			t.Fatalf("expected signingMethod %v, got %v", JWT_HS256.signingMethod, got.signingMethod)
		}
	})
}
