...
const textCodecTestName = "ECDSA_with_SHA256_over_P256"
...
func TestMethod_MarshalText(t *testing.T) {
	...
	t.Run("returns the registry name", func(t *testing.T) {
		...
		data, err := ECDSA_with_SHA256_over_P256.MarshalText()
		if err != nil {
...
func TestMethod_UnmarshalText(t *testing.T) {
	...
	t.Run("returns ErrAlgorithmNotSupported for unknown name", func(t *testing.T) {
		...
		if domErr.Type != EcdsaMethod {
			t.Fatalf("expected type %q, got %q", EcdsaMethod, domErr.Type)
		}
	})
}
...
func TestMethod_TextCodec_RoundTrip(t *testing.T) {
	...
	t.Run("MarshalText then UnmarshalText preserves identity", func(t *testing.T) {
		...
		if got.Name() != ECDSA_with_SHA256_over_P256.Name() {
			t.Fatalf("expected %q, got %q", ECDSA_with_SHA256_over_P256.Name(), got.Name())
		}

		if got.kind != ECDSA_with_SHA256_over_P256.kind {
			t.Fatalf("expected kind %v, got %v", ECDSA_with_SHA256_over_P256.kind, got.kind)
		}

		if got.curve != ECDSA_with_SHA256_over_P256.curve {
			t.Fatalf("expected curve %v, got %v", ECDSA_with_SHA256_over_P256.curve, got.curve)
		}
	})
}
...
func TestMethod_JSON_RoundTrip(t *testing.T) {
	...
	t.Run("encoding/json honors TextMarshaler/TextUnmarshaler", func(t *testing.T) {
		...
		in := Config{Algo: ECDSA_with_SHA256_over_P256}
		...
		const expected = `{"algo":"ECDSA_with_SHA256_over_P256"}`
