...
const textCodecTestName = "SHA256"
...
func TestMethod_TextCodec_RoundTrip(t *testing.T) {
	...
	t.Run("MarshalText then UnmarshalText preserves identity", func(t *testing.T) {
		...
		if got.Name() != SHA256.Name() {
			t.Fatalf("expected %q, got %q", SHA256.Name(), got.Name())
		}

		if got.kind != SHA256.kind {
			t.Fatalf("expected kind %v, got %v", SHA256.kind, got.kind)
		}
	})
}
