...
Conventions and standards for all packages under `modules/core/common`:
...
5. **Options** - the fields of the struct should be private. Check a func With<FieldName> to set the field. Each With function must validate its input using `if valid { assign }` — never use guard clauses with early `return` inside the option closure. Invalid input is silently ignored, preserving the default.
   ...
   ```
6. **Preconfigured Default Singletons** - Check if having a default singleton is necessary or adds value. Check `modules/core/crypto` package for examples.
...
All imports of `github.com/guidomantilla/yarumo/core/common/<pkg>` must use the alias `c<last-segment>`:
...
// Bad — missing alias
import (
    "github.com/guidomantilla/yarumo/core/common/assert"
    "github.com/guidomantilla/yarumo/core/common/errs"
)
```
...
- `crypto/random` → `crandom` (canonical — frequent inside `modules/core/crypto/*`).
...
cd modules/core/common && go vet -vettool=/tmp/inlineassign ./...
```
...
own module: see [`modules/core/crypto/CODING_STANDARDS.md`](../crypto/CODING_STANDARDS.md).
Crypto is no longer part of `modules/core/common`.

## `common/lifecycle/` — Lone Goroutine-Dispatching Exception

`modules/core/common/` follows the principle "pure libraries, no lifecycle, no
...
Every other package under `modules/core/common/` MUST remain free of:
