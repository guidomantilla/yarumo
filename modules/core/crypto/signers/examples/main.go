...
import (
	...
	chmacs "github.com/guidomantilla/yarumo/core/crypto/signers/hmacs"
	crsassas "github.com/guidomantilla/yarumo/core/crypto/signers/rsassas"
...
// rsassasExample demonstrates RSASSA key generation, signing, and verification
// with PSS and PKCS1v15 padding schemes.
func rsassasExample(data []byte) {
	fmt.Println("=== RSASSA ===")

	// PSS / SHA-256 with 2048-bit key
	pssKey, err := crsassas.RSASSA_PSS_using_SHA256.GenerateKey(2048)
	if err != nil {
		log.Fatalf("RSASSA-PSS SHA256 key generation failed: %v", err)
	}

	sig, err := crsassas.RSASSA_PSS_using_SHA256.Sign(pssKey, data)
	if err != nil {
		log.Fatalf("RSASSA-PSS SHA256 sign failed: %v", err)
	}

	fmt.Printf("RSASSA-PSS/SHA256 (2048) Signature (%d bytes)\n", len(sig))

	ok, err := crsassas.RSASSA_PSS_using_SHA256.Verify(&pssKey.PublicKey, sig, data)
	if err != nil {
		log.Fatalf("RSASSA-PSS SHA256 verify failed: %v", err)
	}

	fmt.Printf("RSASSA-PSS/SHA256 Verify: %v\n\n", ok)

	// PKCS1v15 / SHA-256 with 2048-bit key
	pkcsKey, err := crsassas.RSASSA_PKCS1v15_using_SHA256.GenerateKey(2048)
	if err != nil {
		log.Fatalf("RSASSA-PKCS1v15 SHA256 key generation failed: %v", err)
	}

	sig, err = crsassas.RSASSA_PKCS1v15_using_SHA256.Sign(pkcsKey, data)
	if err != nil {
		log.Fatalf("RSASSA-PKCS1v15 SHA256 sign failed: %v", err)
	}

	fmt.Printf("RSASSA-PKCS1v15/SHA256 (2048) Signature (%d bytes)\n", len(sig))

	ok, err = crsassas.RSASSA_PKCS1v15_using_SHA256.Verify(&pkcsKey.PublicKey, sig, data)
	if err != nil {
		log.Fatalf("RSASSA-PKCS1v15 SHA256 verify failed: %v", err)
	}
...
func registryExample() {
	...
	// Look up an RSASSA method by name
	rsa, err := crsassas.Get("RSASSA_PSS_using_SHA256")
	if err != nil {
		log.Fatalf("rsassas registry lookup failed: %v", err)
	}

	fmt.Printf("crsassas.Get: %s\n", rsa.Name())

	// List all registered RSASSA methods
	fmt.Println("crsassas.Supported:")

	for _, m := range crsassas.Supported() {
		fmt.Printf("  - %s\n", m.Name())
	}
	...
	// Non-existent algorithm (rsassas)
	_, err = crsassas.Get("UNKNOWN")
	fmt.Printf("crsassas.Get(\"UNKNOWN\") error: %v\n", err)
