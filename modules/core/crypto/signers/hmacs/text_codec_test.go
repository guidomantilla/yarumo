...
const textCodecTestName = "HMAC_with_SHA256"
...
func TestMethod_MarshalText(t *testing.T) {
	...
	t.Run("returns the registry name", func(t *testing.T) {
		...
		data, err := HMAC_with_SHA256.MarshalText()
		if err != nil {
...
func TestMethod_UnmarshalText(t *testing.T) {
	...
	t.Run("returns ErrAlgorithmNotSupported for unknown name", func(t *testing.T) {
		...
		if domErr.Type != HmacMethod {
			t.Fatalf("expected type %q, got %q", HmacMethod, domErr.Type)
		}
	})
}
...
func TestMethod_TextCodec_RoundTrip(t *testing.T) {
	...
	t.Run("MarshalText then UnmarshalText preserves identity", func(t *testing.T) {
		...
		if got.Name() != HMAC_with_SHA256.Name() {
			t.Fatalf("expected %q, got %q", HMAC_with_SHA256.Name(), got.Name())
		}

		if got.kind != HMAC_with_SHA256.kind {
			t.Fatalf("expected kind %v, got %v", HMAC_with_SHA256.kind, got.kind)
		}

		if got.keySize != HMAC_with_SHA256.keySize {
			t.Fatalf("expected keySize %d, got %d", HMAC_with_SHA256.keySize, got.keySize)
		}
	})
}
...
func TestMethod_JSON_RoundTrip(t *testing.T) {
	...
	t.Run("encoding/json honors TextMarshaler/TextUnmarshaler", func(t *testing.T) {
		...
		in := Config{Algo: HMAC_with_SHA256}
		...
		const expected = `{"algo":"HMAC_with_SHA256"}`
