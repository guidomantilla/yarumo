...
From any worktree, anchored at the `modules/core/crypto` go.mod:

```bash
# Tokens
go test -run=^$ -fuzz=^FuzzValidate$         -fuzztime=30s ./tokens/
go test -run=^$ -fuzz=^FuzzDecodeUnsafe$     -fuzztime=30s ./tokens/

# Certs
go test -run=^$ -fuzz=^FuzzClientTlsFromPEM$    -fuzztime=30s ./certs/
go test -run=^$ -fuzz=^FuzzServerTlsFromPEM$    -fuzztime=30s ./certs/
go test -run=^$ -fuzz=^FuzzParseCertificatePEM$ -fuzztime=30s ./certs/
go test -run=^$ -fuzz=^FuzzParsePEMChain$       -fuzztime=30s ./certs/
go test -run=^$ -fuzz=^FuzzParseCSR$            -fuzztime=30s ./certs/
```
...
```bash
go test -run=^FuzzValidate$ ./tokens/
```
