...
const textCodecTestName = "RSASSA_PSS_using_SHA256"
...
func TestMethod_MarshalText(t *testing.T) {
	...
	t.Run("returns the registry name", func(t *testing.T) {
		...
		data, err := RSASSA_PSS_using_SHA256.MarshalText()
		if err != nil {
...
func TestMethod_UnmarshalText(t *testing.T) {
	...
	t.Run("returns ErrAlgorithmNotSupported for unknown name", func(t *testing.T) {
		...
		if domErr.Type != RsassasMethod {
			t.Fatalf("expected type %q, got %q", RsassasMethod, domErr.Type)
		}
	})
}
...
func TestMethod_TextCodec_RoundTrip(t *testing.T) {
	...
	t.Run("MarshalText then UnmarshalText preserves identity", func(t *testing.T) {
		...
		if got.Name() != RSASSA_PSS_using_SHA256.Name() {
			t.Fatalf("expected %q, got %q", RSASSA_PSS_using_SHA256.Name(), got.Name())
		}

		if got.kind != RSASSA_PSS_using_SHA256.kind {
			t.Fatalf("expected kind %v, got %v", RSASSA_PSS_using_SHA256.kind, got.kind)
		}

		if got.padding != RSASSA_PSS_using_SHA256.padding {
			t.Fatalf("expected padding %v, got %v", RSASSA_PSS_using_SHA256.padding, got.padding)
		}
	})
}
...
func TestMethod_JSON_RoundTrip(t *testing.T) {
	...
	t.Run("encoding/json honors TextMarshaler/TextUnmarshaler", func(t *testing.T) {
		...
		in := Config{Algo: RSASSA_PSS_using_SHA256}
		...
		const expected = `{"algo":"RSASSA_PSS_using_SHA256"}`
