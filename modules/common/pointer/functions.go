...
// It uses == nil instead of IsNil to only guard against untyped nil.
// A typed nil pointer (e.g. var p *int) is still a pointer and should return true.
...
func IsZero[T cconstraints.Comparable](v T) bool {
	return Zero[T]() == v
}

// IsNotZero returns true if the argument is not a zero value.
func IsNotZero[T cconstraints.Comparable](v T) bool {
	return Zero[T]() != v
}
...
func IsType(v any, typeName string) bool {
	...
	return val.Type().String() == typeName
}
