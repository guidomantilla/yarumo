...
func RandomString(length int, opts ...Option) string {
	...
	b := make([]byte, length)
	for i := range b {
		b[i] = options.charset[rand.IntN(len(options.charset))]
	}

	return string(b)
}

// Substring returns part of a string starting at offset with the given length.
func Substring(str string, offset uint, length uint) string {
	...
	return string(rs[offset : offset+length])
}
...
func Capitalize(str string, options ...Option) string {
	opts := NewOptions(options...)
	return cases.Title(opts.lang).String(str)
}
...
// FilterBy performs in place filtering of a slice based on a predicate.
func FilterBy[T cconstraints.Comparable](slice []T, keep FilterFn[T]) []T {
	if len(slice) == 0 {
		return slice
	}
...
func Count[T cconstraints.Comparable](slice []T) int {
	return len(slice)
}

// CountBy returns the number of elements in a slice that satisfy a given predicate.
func CountBy[T cconstraints.Comparable](slice []T, keep FilterFn[T]) int {
	count := 0
...
func ToMap[T cconstraints.Comparable](slice []T) map[T]int {
...
func ToMapBy[T cconstraints.Comparable](slice []T, keep FilterFn[T]) map[T]int {
...
func In[T cconstraints.Comparable](value T, slice ...T) bool {
	if len(slice) == 0 {
		return false
	}
...
func NotIn[T cconstraints.Comparable](value T, slice ...T) bool {
	return !In(value, slice...)
}

// Every checks if all the values exist in the slice.
func Every[T cconstraints.Comparable](values []T, slice ...T) bool {
	if len(slice) == 0 {
		return false
	}
...
func Some[T cconstraints.Comparable](values []T, slice ...T) bool {
	if len(slice) == 0 {
		return false
	}
...
func None[T cconstraints.Comparable](values []T, slice ...T) bool {
	return !Some(values, slice...)
}

// Union returns a slice containing unique elements from both slices.
func Union[T cconstraints.Comparable](a []T, b []T) []T {
	length := len(a) + len(b)
...
func Intersection[T cconstraints.Comparable](a []T, b []T) []T {
	var result []T
...
func Difference[T cconstraints.Comparable](a []T, b []T) ([]T, []T) {
	var (
		left  []T
		right []T
	)
...
func Map[T, U cconstraints.Comparable](slice []T, fn ItemMapFn[T, U]) []U {
	trueFilter := func(x T) bool {
		return true
	}
...
func MapBy[T, U cconstraints.Comparable](slice []T, fn ItemMapFn[T, U], keep FilterFn[T]) []U {
...
func Copy[T cconstraints.Comparable](slice []T) []T {
	if slice == nil {
		return cpointer.Zero[[]T]()
	}
...
func Reverse[T cconstraints.Comparable](slice []T) []T {
	slices.Reverse(slice)
	return slice
}

// Shuffle shuffles (in place) a slice.
func Shuffle[T cconstraints.Comparable](slice []T) []T {
	if len(slice) <= 1 {
		return slice
	}
...
func Deduplicate[T cconstraints.Comparable](slice []T) []T {
	if len(slice) < 2 {
		return slice
	}
...
func Sort[T cconstraints.Ordenable](slice []T) []T {
	slices.Sort(slice)
	return slice
}

// Chunk returns an array of slices split into groups the length of size.
func Chunk[T cconstraints.Comparable](slice []T, size int) [][]T {
	if size <= 0 {
		return cpointer.Zero[[][]T]()
	}
...
// Delete deletes the element at the specified index from a slice.
func Delete[T cconstraints.Comparable](idx int, slice []T) []T {
	if len(slice) == 0 {
		return cpointer.Zero[[]T]()
	}
...
// DeleteRange deletes the elements between start and end index (inclusive) from a slice.
func DeleteRange[T cconstraints.Comparable](start, end int, slice []T) []T {
	if len(slice) == 0 {
		return cpointer.Zero[[]T]()
	}
...
func Push[T cconstraints.Comparable](slice []T, value T) []T {
	return append(slice, value)
}

// Pop removes the last element from a slice and returns it along with the modified slice.
func Pop[T cconstraints.Comparable](slice []T) (T, []T) {
	if len(slice) == 0 {
		return cpointer.Zero[T](), cpointer.Zero[[]T]()
	}
...
func Max[T cconstraints.Ordenable](slice []T) T {
	if len(slice) == 0 {
		return cpointer.Zero[T]()
	}
...
func Min[T cconstraints.Ordenable](slice []T) T {
	if len(slice) == 0 {
		return cpointer.Zero[T]()
	}
...
func HasKey[K cconstraints.Comparable, V any](key K, keyValues map[K]V) bool {
	_, ok := keyValues[key]
	return ok
}

// Keys returns a slice of keys from the provided maps.
func Keys[K cconstraints.Comparable, V any](keyValues ...map[K]V) []K {
	var size int
...
func UniqueKeys[K cconstraints.Comparable, V any](keyValues ...map[K]V) []K {
	var size int
	for i := range keyValues {
		size += len(keyValues[i])
	}
...
func Values[K cconstraints.Comparable, V any](keyValues ...map[K]V) []V {
	var size int
	for i := range keyValues {
		size += len(keyValues[i])
	}
...
func UniqueValues[K cconstraints.Comparable, V cconstraints.Comparable](keyValues ...map[K]V) []V {
	var size int
	for i := range keyValues {
		size += len(keyValues[i])
	}
...
func PickByKeys[K cconstraints.Comparable, V any](keyValues map[K]V, keys []K) map[K]V {
	r := make(map[K]V)

	for i := range keys {
		v, ok := keyValues[keys[i]]
		if ok {
			r[keys[i]] = v
		}
	}
...
func PickByValues[K cconstraints.Comparable, V cconstraints.Comparable](keyValues map[K]V, values []V) map[K]V {
	r := make(map[K]V)
...
func OmitByKeys[K cconstraints.Comparable, V any](keyValues map[K]V, keys []K) map[K]V {
	r := make(map[K]V)
	for k := range keyValues {
		r[k] = keyValues[k]
	}
...
func OmitByValues[K cconstraints.Comparable, V cconstraints.Comparable](keyValues map[K]V, values []V) map[K]V {
	r := make(map[K]V)
