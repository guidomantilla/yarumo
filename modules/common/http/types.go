...
//
// Timeouts alignment:
// - The underlying http.Client.Timeout is configurable via Options and acts as
//   an overall per-request timeout.
// - The internal rate limiter wait is bounded by the effective deadline, which
//   is the minimum between req.Context() deadline and the client Timeout.
// - When a *http.Transport is provided, selected transport timeouts are capped
//   to not exceed the client Timeout (TLSHandshakeTimeout, ResponseHeaderTimeout,
//   ExpectContinueTimeout). Stricter values provided by the transport are kept.
//
