...
- [x] Revisar contrato de interfaz (`Client`), garantizando compatibilidad y claridad.
- [x] Validar semántica del `rate.Limiter` (`Inf` = deshabilitado, `burst` > 0).
- [x] Confirmar correcto cierre de `Body` en todas las rutas (éxito/error/retry).
- [x] Alinear timeouts (`Client.Timeout`, `Transport` y `Context`).
...
- [x] Contrato estable de interfaz (para mocks/tests).
