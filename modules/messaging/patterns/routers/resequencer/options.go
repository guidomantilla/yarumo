...
// Option is a functional option for configuring resequencer Options.
// Resequencer has no T-typed options, so Option is non-generic —
// matching bridge/filter/history/barrier and diverging from
// router/Option[T] is intentional.
type Option func(opts *Options)

// Options holds the configuration for a Resequencer.
type Options struct {
...
// WithGroupTimeout because unbounded buffering is a memory leak. The
// default maxGroups is DefaultMaxGroups; the default sweepInterval is
// DefaultSweepInterval; the default ErrorHandler is
...
// WithGroupTimeout sets the per-group timeout. A group whose missing
// position is not received within this duration is evicted by the
// sweeper (every still-buffered message in the group fires
// WithDropHandler). The constructor REQUIRES a positive value to
// bound memory. Non-positive values are ignored.
...
// Resequencer tracks at any one time. New correlations arriving while
// the in-flight group count is already at the cap are dropped via
...
// forward Send failure during emit. The default (when
...
// intentional drop (missing/invalid sequence metadata, MaxGroups cap,
// duplicate sequence number, quorum timeout, Stop drain). The default
// (when WithDropHandler is not passed) is nil — intentional drops are
// silent. Wire this to ship throughput metrics or audit trails. Nil
// arguments are ignored (the previously installed handler is
// preserved).
