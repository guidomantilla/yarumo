...
// Option is a functional option for configuring wiretap Options.
// Wiretap has no T-typed options, so Option is non-generic — matching
// bridge/filter and diverging from router/Option[T] is intentional and
// keeps the callsite simpler.
type Option func(opts *Options)

// Options holds the configuration for a Wiretap.
type Options struct {
...
// wiretap-observed failure: primary destination Send failure
// (ErrForwardFailed) AND tap Send failure (ErrTapSendFailed). Tap
// failures are reported only — they never alter the primary flow. The
