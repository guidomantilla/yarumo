# Coding Standards — modules/extensions/telemetry/otel/http/
...
### Override: Bridge module (extensions/telemetry/otel/<X>/ pattern)

This module is a bridge between an HTTP client middleware and OTel
signals. It pairs `extensions/telemetry/otel/slog/` (bridge between slog
and OTel logs) — same pattern: workspace concern ↔ OTel. It does NOT
belong under `modules/extensions/common/http/` (where `limiter/` and
...
Both transports follow the `extensions/common/http/{limiter,retry}/`
