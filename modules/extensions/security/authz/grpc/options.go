...
// Options holds the configuration for the Require unary + stream
// interceptors.
type Options struct {
	principalReader authz.PrincipalReader
	auditHook       authz.AuditHookFn
	resourceFn      GRPCResourceResolverFn
}
...
// is nil by default — Require fails closed when invoked without an
...
// WithResourceResolver installs the resolver the interceptor uses to
// compute the Resource a gRPC RPC targets. Nil resolvers are ignored.
// When no resolver is configured, the interceptor populates
// Request.Resource with the zero value — useful for action-only checks
// where resource fields carry no meaning.
func WithResourceResolver(resolver GRPCResourceResolverFn) Option {
	return func(opts *Options) {
		if resolver != nil {
