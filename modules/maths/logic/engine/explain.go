...
func explainWithRules(f p.Formula, facts FactBase, rules []Rule, seen map[p.Var]bool) (*Explain, bool) {
	switch x := f.(type) {
	case p.TrueF:
...
func findSupportingRule(v p.Var, facts FactBase, rules []Rule) (*Rule, bool) {
	for i := range rules {
		r := &rules[i]
		if r.then == v && r.when.Eval(p.Fact(facts)) {
			return r, true
		}
	}
